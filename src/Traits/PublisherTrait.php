...
trait PublisherTrait
{
    ...
    private function publishController()
    {
        ...
        if ($this->publishFile($sourceStub, $destinationPath, $destinationFile)) {
            $this->stdOut['routes']['web'][] = sprintf(
                "Route::resource('%s', '%sController');",
                $this->variables['ROUTES'],
    ...
    private function publishApiController()
    {
        ...
        if ($this->publishFile($sourceStub, $destinationPath, $destinationFile)) {
            $this->stdOut['routes']['api'][] = sprintf(
                "Route::resource('%s', 'Api\%sController');",
                $this->variables['ROUTES'],
                $this->variables['MODEL']
            );
        }
        ...
        collect($models)->each(function ($model) use ($modelName) {
            ...
            if (in_array(strtolower($model['model']), $validKeys)) {
                $this->variables = [
                    ...
                    'UPDATE_EXCEPTIONS' => $this->varExport($model['update']['exclude']),
                    'STORE_EXCEPTIONS' => $this->varExport($model['store']['exclude']),
                    'UPDATE_VALIDATION' => $this->varExport($model['update']['validation']),
                    'STORE_VALIDATION' => $this->varExport($model['store']['validation']),
                ];
            }
        });
    }
    ...
    private function getDisplayVariable($field, $display = "index")
    {
        ...
        if ($field->getViews()[$display] === 'readonly') {
            $sourceStub = dirname(dirname(__FILE__)) . sprintf(
                '/stubs/views/partials/fields/%s/_readonly.blade.php.stub',
                $display,
                $field->getType()
            );
        } else {
            $sourceStub = dirname(dirname(__FILE__)) . sprintf(
                '/stubs/views/partials/fields/%s/%s.blade.php.stub',
                $display,
                $field->getType()
            );
            if (!$this->file->isFile($sourceStub)) {
                $sourceStub = dirname(dirname(__FILE__)) . sprintf(
                    '/stubs/views/partials/fields/%s/_general.blade.php.stub',
                    $display
                );
            }
        }
        ...
        if (!empty($field->getOptions()) && ($display === 'create' || $display === 'edit')) {
            $field_options = '';
            $sourceOptionStub = dirname(dirname(__FILE__)) . sprintf(
                '/stubs/views/partials/fields/%s/%s-option.blade.php.stub',
                $display,
                $field->getType()
            );
        ...
        $stub = str_replace(
            '{MODEL_VARIABLE}',
            sprintf('$%s->%s', $this->variables['VARIABLE'], $field->getColumnName()),
            $stub
        );
